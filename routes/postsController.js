...
/**
 * @swagger
 *  /menu/{Field name}/{info}:
 *   get:
 *    tags: [Menu]
 *    summary : Find menu by info
 *    security:
 *     - ApiKeyAuth: []
 *    description: Return a single element
 *    parameters:
 ...
 *       in: path
 *       type: string
 *       description: Id or title of the menu
 *       required: true
 *    responses:
 *      '200':
 *          description : A successful response
 ...
 *          description: Tax not found
 */
router.get("/menu/:tags/:id", (req, res) => {
  //Select one article : OK
  let query = {};
  query[req.params.tags] = req.params.id;
  Menu.find(query, (err, docs) => {
    if (!err) res.send(docs);
    else console.log("Error : couldn't retrieve data " + err);
  });
});
 ...
 *            required: true
 *          - name: info
 *            description: ID, title or rate of the menu
 *            in: path
 *            type: string
 *            required: true
 *          - name: requestBody
...
router.put("/menu/update/:tags/:id", (req, res) => {
  // Update : De mbola très bien koa
...
router.delete("/menu/delete/:tags/:id", (req, res) => {
  ...
  let query = {};
  query[req.params.tags] = req.params.id;
  Menu.deleteMany(query, function (err, obj) {
    if (err) throw err;
    res.send(
      `The category with the ${req.params.tags} : ${req.params.id} has been deleted from the database.`
    );
